 ...
 */
Array<MeasureCandidate> MyAssembleCandidates(const std::vector<Schedule>& picks) {
...
/*!\brief A search strategy that generates measure candidates using my search. */
class MySearchNode : public SearchStrategyNode {
 public:
  ...
  struct State {
    ...
    /*! \brief An interface method to be called by it's counterpart in MySearchNode */
  ...
  static void RegisterReflection() {
    ...
    refl::ObjectDef<MySearchNode>()
        .def_ro("population_size", &MySearchNode::population_size)
        .def_ro("num_empty_iters_before_early_stop",
                &MySearchNode::num_empty_iters_before_early_stop)
        .def_ro("init_measured_ratio", &MySearchNode::init_measured_ratio)
        .def_ro("init_min_unmeasured", &MySearchNode::init_min_unmeasured)
        .def_ro("max_fail_count", &MySearchNode::max_fail_count)
        .def_ro("genetic_num_iters", &MySearchNode::genetic_num_iters)
        .def_ro("genetic_mutate_prob", &MySearchNode::genetic_mutate_prob)
        .def_ro("genetic_max_fail_count", &MySearchNode::genetic_max_fail_count)
        .def_ro("eps_greedy", &MySearchNode::eps_greedy);
  }

  static constexpr const char* _type_key = "meta_schedule.MySearch";
  TVM_DECLARE_FINAL_OBJECT_INFO(MySearchNode, SearchStrategyNode);
  ...
  SearchStrategy Clone() const final {
    ObjectPtr<MySearchNode> n = make_object<MySearchNode>();
...
std::vector<Schedule> MySearchNode::State::SampleInitPopulation(int num) {
  auto _ = Profiler::TimedScope("MySearch/SampleInitPopulation");
  ThreadedTraceApply pp(self->postprocs_);
...
std::vector<Schedule> MySearchNode::State::EvolveWithCostModel(std::vector<Schedule> population,
                                                               int num) {
  IRModuleSet exists(database_->GetModuleEquality());
  {
    auto _ = Profiler::TimedScope("MySearch/Evolve/Misc/CopyMeasuredWorkloads");
    ICHECK_GT(num, 0);
...
std::vector<Schedule> MySearchNode::State::PickWithEpsGreedy(
    const std::vector<Schedule>& unmeasured, const std::vector<Schedule>& bests, int num) {
  auto _ = Profiler::TimedScope("MySearch/PickWithEpsGreedy");
...
Optional<Array<MeasureCandidate>> MySearchNode::State::GenerateMeasureCandidates() {
  if (st >= max_trials) {
    return std::nullopt;
  }
  ...
  return MyAssembleCandidates(picks);
}

void MySearchNode::State::NotifyRunnerResults(const Array<MeasureCandidate>& measure_candidates,
                                              const Array<RunnerResult>& results) {
  st += results.size();
  ed += results.size();
}

size_t MySearchNode::State::ModuleHash(const IRModule& mod) const {
  return database_->GetModuleEquality().Hash(mod);
}

SearchStrategy SearchStrategy::MySearch(int population_size,         //
                                        double init_measured_ratio,  //
                                        int init_min_unmeasured,     //
                                        int max_fail_count,          //
                                        int genetic_num_iters,       //
...
class MySearch : public SearchStrategy {
 public:
  TVM_DEFINE_MUTABLE_NOTNULLABLE_OBJECT_REF_METHODS(MySearch, SearchStrategy, MySearchNode);
};

Array<Schedule> MySearchSampleInitPopulation(MySearch self, int num) {
  std::vector<Schedule> results = self->state_->SampleInitPopulation(num);
  return Array<Schedule>(results.begin(), results.end());
}

Array<Schedule> MySearchEvolveWithCostModel(MySearch self, Array<Schedule> population, int num) {
...
TVM_FFI_STATIC_INIT_BLOCK({ MySearchNode::RegisterReflection(); });

TVM_REGISTER_NODE_TYPE(MySearchNode);
TVM_FFI_REGISTER_GLOBAL("meta_schedule.SearchStrategyMySearch")
    .set_body_typed(SearchStrategy::MySearch);
TVM_FFI_REGISTER_GLOBAL("meta_schedule.SearchStrategyMySearchSampleInitPopulation")
    .set_body_typed(MySearchSampleInitPopulation);
TVM_FFI_REGISTER_GLOBAL("meta_schedule.SearchStrategyMySearchEvolveWithCostModel")
    .set_body_typed(MySearchEvolveWithCostModel);
